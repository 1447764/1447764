## 🚀 About Me
- 주로 C++과 **Unreal Engine** 기반으로 게임 시스템을 설계하고 구현하는 게임 클라이언트 개발자입니다!
- 대규모 프로젝트의 복잡한 구조를 이해하고, 유지보수가 용이한 코드를 작성하기 위해 항상 노력합니다.
- **Python**을 활용한 AI 모델 연동이나 **Django**를 이용한 간단한 백엔드 구축 등 다양한 기술 분야에 호기심을 갖고 도전하는 것을 즐깁니다.

## 🎓 Education

- **중앙대학교** (2018.03 ~ 2023.08)
  - 물리학과 학사
  - 소프트웨어 벤처 융합전공
- **청년취업사관학교 언리얼 과정** (2023.09 ~ 2024.03)
  - 언리얼엔진 XR 개발 과정
- **삼성 청년 SW/AI 아카데미 SSAFY** (2025.07 ~ )

## 🛠️ My Tech Stack
> C++과 **Unreal Engine**을 주력으로 사용합니다

<table>
  <tr>
    <td align="center" width="96">
      <a href="#-my-tech-stack">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="48" height="48" alt="C++" />
      </a>
      <br>C++
    </td>
    <td align="center" width="96">
      <a href="#-my-tech-stack">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/unrealengine/unrealengine-original.svg" width="48" height="48" alt="Unreal Engine" />
      </a>
      <br>Unreal
    </td>
        <td align="center" width="96">
      <a href="#-my-tech-stack" >
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" width="48" height="48" alt="C#" />
      </a>
      <br>C#
    </td>
    <td align="center" width="96">
      <a href="#-my-tech-stack">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/unity/unity-original.svg" width="48" height="48" alt="Unity" />
      </a>
      <br>Unity
    </td>
    <td align="center" width="96">
      <a href="#-my-tech-stack">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="48" height="48" alt="Python" />
      </a>
      <br>Python
    </td>
     <td align="center" width="96">
      <a href="#-my-tech-stack">
        <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg" width="48" height="48" alt="Django" />
      </a>
      <br>Django
    </td>
  </tr>
</table>



## 🚀 Featured Projects

### 📌 [프로젝트 1: 멀티플레이 TPS 게임 'Clash Of Blood'](https://docs.google.com/presentation/d/1uY7ZVTC_kaYKVGXxAOkZK9mKuWxxaJmHRefZkIYyjGc/edit?usp=sharing)
[![Video Label](http://img.youtube.com/vi/xzb595z0GRU/0.jpg)](https://youtu.be/xzb595z0GRU?si=XRK7VGsWDpGogLqD&t=48)
> - **개발 기간:** 2024.01 ~ 2024.03 (2개월)
> - **한 줄 설명:** 바닥을 많이 칠하면 이기는 팀전 멀티플레이 TPS 게임
> - **주요 역할 및 구현 내용:**
>   - C++ 기반의 무기 시스템(발사, 잉크 칠하기) 및 인벤토리 구현
>   - Replication을 활용한 서버 동기화 구현
>   - 블루프린트를 활용한 UI 디자인 및 기능 연동
> - **사용 기술:** `Unreal Engine`, `C++`, `Blueprint`

<br>

### 📌 [프로젝트 2: 캐주얼 멀티플레이 'Hide & Seek'](https://docs.google.com/presentation/d/16WwkorbaNLINQUE8oEaOnAk0nhjjkq_TlBndq1MnVxo/edit?usp=sharing)
[![Video Label](https://img.youtube.com/vi/5Dm7AnNd7-U/0.jpg)](https://youtu.be/5Dm7AnNd7-U?si=6lx_D2yj779AOFEZ)
> - **개발 기간:** 2023.12 ~ 2024.01 (1개월)
> - **한 줄 설명:** 사물로 변한 플레이어를 술래가 찾는 게임
> - **주요 역할 및 구현 내용:**
>   - Replication을 활용한 서버 동기화 구현
> - **사용 기술:** `Unreal Engine`, `C++`, `Blueprint`


## 📊 GitHub Stats
[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=1447764&layout=compact&theme=dark)](https://github.com/anuraghazra/github-readme-stats)


## 📫 Contact Me
<a href="mailto:971m9280@gmail.com"><img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"></a> <img src="https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white"></a>  <a href="https://m-ha.tistory.com/"><a href="https://m-ha.tistory.com/"><img src="https://img.shields.io/badge/Tistory-E65543?style=for-the-badge&logo=tistory&logoColor=white"></a></a>
